...
const ComboBoxFilter = () => {
  ...
  const handleSelect = (newValue: string) => {
    // Jika memilih "All", set ke empty string untuk menampilkan semua
    const finalValue = newValue === "all" ? "" : newValue;
    setSelectValue((prevValue) => (prevValue === finalValue ? "" : finalValue));
    setIsOpen(false);
  };
  ...
  useEffect(() => {
    if (categoryParams) {
      ...
      setSelectValue("all"); // Default ke "All" jika tidak ada category
    }
  }, [categoryParams]);
